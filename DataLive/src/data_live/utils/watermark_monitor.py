# src/data_live/utils/watermark_monitor.py
...
from src.data_live.utils.minio_client import get_sheet_watermarks
from src.data_live.utils.transform_utils import parse_mixed_dates, to_snake_case
...
def data_live_watermark_check(spreadsheet_objects: dict, minio_client, minio_bucket: str) -> pd.DataFrame:
    """Check watermark drift for the 5 data live sheets (matz..imam) using toko grain."""
    watermark_records = get_sheet_watermarks(
        minio_client, minio_bucket, "watermarks/data_live_toko.json"
    )[1]
    sheet_registry = {
        "matz": ("SH_KEY_MATZ", "Performa Sesi Live"),
        "ian":  ("SH_KEY_IAN",  "Performa Sesi Live"),
        "deni": ("SH_KEY_DENI", "Performa Sesi Live"),
        "riwa": ("SH_KEY_RIWA", "Performa Sesi Live"),
        "imam": ("SH_KEY_IMAM", "Performa Sesi Live"),
    }
    return compare_watermark_vs_sheet(
        "Performa Sesi Live", list(sheet_registry.keys()), sheet_registry,
        watermark_records, spreadsheet_objects,
        toko_col="toko", tanggal_col="tanggal",
        required_cols=["waktu_live"],
    )
