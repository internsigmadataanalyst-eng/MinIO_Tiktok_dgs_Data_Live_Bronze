...
def update_sheet_watermarks(minio_client: Minio, bucket: str, watermark_path: str, prev_records: list, sheet_max_dates: dict, sheet_registry: dict | None = None):
    """Rebuilds the watermark table from THIS run's updated sheets only.

    The JSON is cleared first (stale/duplicate rows are dropped): only sheets that
    produced new data this run (sheet_max_dates) are written. updated_at is refreshed
    (now) for every sheet in the run — change only when the sheet is updated, like the
    current behavior. Previous records are used ONLY to recover sheet_name for creds
    ...
    for rec in prev_records:
        ...
        prev_sheet_name.setdefault(str(creds), sheet_name)

    records = []
    for creds, max_date in sheet_max_dates.items():
        records.append({
            "creds": creds,
            "sheet_name": creds_to_name.get(creds) or prev_sheet_name.get(str(creds)) or creds,
            "last_processed_date": max_date,
            "updated_at": now,
        })

    records = sorted(records, key=lambda rec: str(rec["creds"]))
