...
def update_sheet_watermarks(minio_client: Minio, bucket: str, watermark_path: str, prev_records: list, sheet_max_dates: dict):
    ...
    Only sheets that produced new data this run (sheet_max_dates) are written.
    Previous records are used to recover sheet_name for creds.
    """
    ...
    for rec in prev_records:
        ...
        prev_sheet_name.setdefault(creds, rec.get("sheet_name"))

    records = []
    for creds, max_date in sheet_max_dates.items():
        records.append({
            "creds": creds,
            "sheet_name": prev_sheet_name.get(creds) or creds,
